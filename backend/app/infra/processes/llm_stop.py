...
EXPECTED_LINE = "Display VM state after stop"
# The VM LLM-LLama3-server is now deallocated.
OUTPUT_STATUS = "LLM stopped"
...
# azure_client_id, azure_secret ONLY, no ansible_user
async def stop_llm_server(extra_vars: dict):
    output = {}
    logs = []
    extra_vars = add_llm_credentials(extra_vars=extra_vars)
    extra_vars_str = convert_dict_to_string(data=extra_vars)
    payload = f'./playbooks/stop-llm-server.yml --extra-vars "{extra_vars_str}"'
    ...
    try:
        ...
        if "status" in output and output["status"] == OUTPUT_STATUS:
            ...
            logger.error("LLM did not STOP yet.")
