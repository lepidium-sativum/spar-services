...
from .util import create_asyncssh_client, convert_dict_to_string, add_ue_credentials
# from ..config import get_infra_config

OUTPUT_STATUS = "UE started"
# Unreal Engine application has been started with specified parameters
# Unreal Engine application is running with the specified parameters
# Unreal Engine application is NOT running with the specified parameters
EXPECTED_LINE = "Start Unreal Engine application with parameters"
...
async def spin_up_both_ue_instance_and_app(extra_vars: dict, inventory: str):
    output = {}
    logs = []
    extra_vars = add_ue_credentials(extra_vars=extra_vars)
    extra_vars_str = convert_dict_to_string(data=extra_vars)
    payload = f'./playbooks/start-ue-combined.yml -i {inventory} --extra-vars "{extra_vars_str}"'
    ...
    try:
        ...
        if "status" in output and output["status"] == OUTPUT_STATUS:
            ...
            logger.error("UE instance/app FAILED to start.")
