...
# ansible_password ONLY
async def deploy_ue_build(extra_vars: dict, s3_object_key: str):
    output = {}
    logs = []
    extra_vars = add_ue_credentials(extra_vars=extra_vars)
    extra_vars_str = convert_dict_to_string(data=extra_vars)
    payload = f'./playbooks/ue-deploy.yml --extra-vars "{extra_vars_str} s3_object_key={s3_object_key}"'
    command = f"cd ~/spar-iac/ansible-aws && ansible-playbook {payload}"
    try:
        ...
        if "status" in output and output["status"] == OUTPUT_STATUS:
            ...
            logger.error("UE did not start successfully.")
