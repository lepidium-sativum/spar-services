...
async def start_ue_app_only(extra_vars: dict, inventory: str):
    output = {}
    logs = []
    extra_vars = add_ue_credentials(extra_vars=extra_vars)
    extra_vars_str = convert_dict_to_string(data=extra_vars)
    payload = f'./playbooks/start-ue-application.yml -i {inventory} --extra-vars "{extra_vars_str}"'
    ...
    try:
        ...
        if "status" in output and output["status"] == OUTPUT_STATUS:
            ...
            logger.error("UE FAILED to start.")
