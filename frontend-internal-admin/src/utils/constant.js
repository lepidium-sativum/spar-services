...
export const sparDurationConvertion = (time) => {
...
export const formatDate = (isoString) => {
  ...
  return `${day}${getDaySuffix(day)} of ${monthName} @ ${hour}h${minute}`;
};

export const conversionString = (conversationn) => {
  const conversation = JSON.parse(conversationn);
  const formattedData = conversation.map((item) => {
    return { role: item.role.trim(), content: item.content.trim() };
  });
...
export const checkGradeLevel = (grade) => {
  if (grade >= 70) {
    return "bg-[#95EBA3]";
...
export const mergeTimelines = (videoData, timeLineConversation) => {
  ...
  // console.log("return conversation: ", timeLineConversation);
